...
trait UDateTimeFormats {
  ...
  implicit lazy val writes: Writes[SSDateTime.TimeZone] = new Writes[SSDateTime.TimeZone] {
  ...
  implicit val uDateTimeInstantWrites: Writes[SSDateTime.Instant] = new Writes[SSDateTime.Instant] {
    def writes(d: SSDateTime.Instant): JsValue = JsNumber(d.millis)
  }

  implicit val uDateTimeInstantFormat: Format[SSDateTime.Instant] = Format[SSDateTime.Instant](
    uDateTimeInstantReads, uDateTimeInstantWrites
  )
